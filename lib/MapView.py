...
class GpsApp:
    ...
    def __init__(self):
        ...
        self.comm = Comm.Comm("opennetmap.org", "/api/")
        self.activate()
...
class GpsTrackTab(BaseInfoTab):
    ...
    def download_osm(self):
        ...
        params = {}
        params["lat"] = "%.6f" % pos["lat"]
        params["lon"] = "%.6f" % pos["lon"]
        e32.ao_sleep(0.05) # let the querypopup disappear
        ip.show(u"Downloading OSM data...", (50, 50), 60000, 100, appuifw.EHLeftVTop)
        data, response = self.Main.comm._send_request("get_osm", params)
        ip.hide()
        self.roads = data["geojson"]["features"]
        #for road in data["geojson"]["features"]:
        #    self.roads.append(road["geometry"]["coordinates"][0])
        #print self.roads
...
#                                                                         p["speed"]*20, p["heading"])
                ...
                pass
        # draw some openstreetmap data on the canvas
        if self.Main.LongOrigin:
            #print self.Main.LongOrigin
            for feature in self.roads: # in a:
                road = feature["geometry"]["coordinates"][0]
                #text = road
                for i in range(0, len(road) - 1):
                    ...
                    self.ui.line([p["x"]+center_x, p["y"]+center_y, 
                                  p2["x"]+center_x, p2["y"]+center_y], outline=0x0000cc, width=2)
                    
                    self.ui.point([p["x"]+center_x, p["y"]+center_y], outline=0x000066, width=3)
                name = feature["properties"]["name"] or u"noname"
                self.ui.text(([p["x"]+center_x+10, p["y"]+center_y+5]), u"%s" % name, font=(u"Series 60 Sans", 10), fill=0x000000)
                self.ui.text(([p["x"]+center_x+11, p["y"]+center_y+6]), u"%s" % name, font=(u"Series 60 Sans", 10), fill=0xffffff)
#                #print p
