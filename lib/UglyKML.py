...
(Sorry, but isn't very useful module, this does just one thing.)
"""
...
class UglyAndHackyKMLExporterButHeyItWorks:
    ...
    def handle_trackpoint(self, linedata):
        ...
        if self.start_folder_flag:
            self.buf.append(start_placemark(ts))
            self.start_folder_flag = 0
        if "gpstime" not in linedata: linedata["gpstime"] = 0
    ...
    def get_zip(self, filename):
        ...
        #filename = os.path.join(u"E:\\", filename)
        name = "all.kml"
